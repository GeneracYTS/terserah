...
`
    ...
    switch (command) {
        case 'menfesleave': {
            ...
            if (other) await this.sendButton(other, room.b.replace(/[@ .+-]/g, '') + ' *Meninggalkan chat*', author, null, [['Mulai Menfes', '.menfesstart']], null)
        ...
        case 'menfesstart': {
            ...
            if (room) {
                room.b = m.sender
                room.state = 'CHATTING'
                await this.sendButton(room.a, '*Menfes Chat Tersambung!*\nDengan: ' + m.sender.replace(/[@ .+-]/g, ''), author, null, [['Hapus Menfes', '.menfesleave']], null)
                await this.sendButton(m.sender, '*Menfes Chat Tersambung!*\nDengan: ' + room.a.replace(/[@ .+-]/g, ''), author, null, [['Hapus Menfes', '.menfesleave']], null)
            } else {
            // Batas
	if (!m.quoted) {
